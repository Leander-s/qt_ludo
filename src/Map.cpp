...
const QVector2D Map::getCoords(const quint8 figure,
                               const quint8 position) const {
        return posCoordMap[getTotalIndex(figure, position)];
}

const quint8 Map::getTotalIndex(const quint8 figure,
                                const quint8 position) const {
        return paths[figure][position];
}

const MapConfig Map::getMapConfig() const {
        if (!initialized) {
                std::cout << "Map is not initialized! Cannot return valid "
                             "config! Call map.initialize() first!"
                          << std::endl;
        }
        return config;
}
